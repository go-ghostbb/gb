...
func Scan(params interface{}, pointer interface{}, paramKeyToAttrMap ...map[string]string) (err error) {
	...
	if pointerType == nil {
		return gberror.NewCode(gbcode.CodeInvalidParameter, "parameter pointer should not be nil")
	}
...
func doScanList(
	...
	if bindToAttrName == "" {
		return gberror.NewCode(gbcode.CodeInvalidParameter, `bindToAttrName should not be empty`)
	}
	...
	for i := 0; i < arrayValue.Len(); i++ {
		...
		if len(relationDataMap) > 0 && !relationFromAttrValue.IsValid() {
			return gberror.NewCodef(gbcode.CodeInvalidParameter, `invalid relation fields specified: "%v"`, relationFields)
		}
		...
		if relationFields != "" && !relationBindToFieldNameChecked {
			...
			if !relationFromAttrField.IsValid() {
				var (
					fieldMap, _ = gbstructs.FieldMap(gbstructs.FieldMapInput{
		...
		default:
			return gberror.NewCodef(gbcode.CodeInvalidParameter, `unsupported attribute type: %s`, bindToAttrKind.String())
		}
	}
