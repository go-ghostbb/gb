...
func (v *Validator) doCheckMap(ctx context.Context, params interface{}) Error {
	...
	inputParamMap := gbconv.Map(params)
	...
	if v.bail && len(errorMaps) > 0 {
		return newValidationError(gbcode.CodeValidationFailed, nil, errorMaps)
	}
	...
	if len(errorMaps) > 0 {
		return newValidationError(gbcode.CodeValidationFailed, checkRules, errorMaps)
	}
	return nil
}
