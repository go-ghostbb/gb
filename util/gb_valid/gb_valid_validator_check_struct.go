...
func (v *Validator) doCheckStruct(ctx context.Context, object interface{}) Error {
	...
	fieldMap, err := gbstructs.FieldMap(gbstructs.FieldMapInput{
		Pointer:          object,
	...
	// It here must use gbstructs.TagFields not gbstructs.FieldMap to ensure error sequence.
	tagFields, err := gbstructs.TagFields(object, structTagPriority)
	if err != nil {
		return newValidationErrorByStr(internalObjectErrRuleName, err)
	}
	...
	if v.assoc == nil || !v.useAssocInsteadOfObjectAttributes {
		...
		inputParamMap = gbconv.Map(v.assoc)
	}
	...
	for _, field := range tagFields {
		...
		if _, ok := inputParamMap[name]; !ok {
			if !v.useAssocInsteadOfObjectAttributes {
				...
				if name != fieldName {
					if foundKey, foundValue := gbutil.MapPossibleItemByKey(inputParamMap, fieldName); foundKey != "" {
		...
		if _, ok := nameToRuleMap[name]; !ok {
			if _, ok = nameToRuleMap[fieldName]; ok {
				...
				if fieldValue := field.Value.Interface(); fieldValue != nil {
					_, isMeta = fieldValue.(gbmeta.Meta)
				}
				checkRules = append(checkRules, fieldRule{
					Name:      name,
					Rule:      rule,
	...
	if v.bail && len(errorMaps) > 0 {
		return newValidationError(gbcode.CodeValidationFailed, resultSequenceRules, errorMaps)
	}
	...
	for _, checkRuleItem := range checkRules {
		...
		if value != nil {
			...
			case reflect.Struct, reflect.Map:
				if gbconv.String(value) == emptyJsonObjectStr {
					value = ""
				}
			case reflect.Slice, reflect.Array:
				if gbconv.String(value) == emptyJsonArrayStr {
					value = ""
				}
			}
		}
		...
		if validatedError := v.doCheckValue(ctx, doCheckValueInput{
			...
			// ============================================================
			if !checkRuleItem.IsMeta && (value == nil || gbconv.String(value) == "") {
				required := false
				// rule => error
...
func getPossibleValueFromMap(inputParamMap map[string]interface{}, fieldName, aliasName string) (value interface{}) {
	_, value = gbutil.MapPossibleItemByKey(inputParamMap, fieldName)
	if value == nil && aliasName != "" {
		_, value = gbutil.MapPossibleItemByKey(inputParamMap, aliasName)
	}
	return
}
