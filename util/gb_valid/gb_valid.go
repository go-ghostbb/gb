// Package gbvalid implements powerful and useful data/form validation functionality.
package gbvalid
...
const (
	...
	ruleMessagePrefixForI18n  = "gb.valid.rule."      // prefix string for each rule configuration in i18n content.
	noValidationTagName       = gbtag.NoValidation    // no validation tag name for struct attribute.
...
var (
	...
	structTagPriority = []string{gbtag.Valid, gbtag.ValidShort}

	// aliasNameTagPriority specifies the alias tag priority array.
	aliasNameTagPriority = []string{gbtag.Param, gbtag.ParamShort}
...
func ParseTagValue(tag string) (field, rule, msg string) {
	...
	match, _ := gbregex.MatchString(`\s*((\w+)\s*@){0,1}\s*([^#]+)\s*(#\s*(.*)){0,1}\s*`, tag)
	if len(match) > 5 {
