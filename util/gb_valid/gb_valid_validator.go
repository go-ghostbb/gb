...
type Validator struct {
	i18nManager                       *gbi18n.Manager     // I18n manager for error message translation.
...
func New() *Validator {
	return &Validator{
		i18nManager: gbi18n.Instance(),         // Use default i18n manager.
...
func (v *Validator) Run(ctx context.Context) Error {
	...
	return v.doCheckValue(ctx, doCheckValueInput{
		...
		Rule:      gbconv.String(v.rules),
		Messages:  v.messages,
		DataRaw:   v.assoc,
		DataMap:   gbconv.Map(v.assoc),
	})
}
...
func (v *Validator) I18n(i18nManager *gbi18n.Manager) *Validator {
	if i18nManager == nil {
		return v
	}
...
func (v *Validator) checkRuleRequired(rule string) bool {
	...
	if gbstr.HasPrefix(rule, requiredRulesPrefix) {
		return true
	}
