// Package gbdes provides useful API for DES encryption/decryption algorithms.
package gbdes
...
func EncryptECB(plainText []byte, key []byte, padding int) ([]byte, error) {
	...
	if err != nil {
		err = gberror.WrapCodef(gbcode.CodeInvalidParameter, err, `des.NewCipher failed for key "%s"`, key)
		return nil, err
	}
...
func EncryptECBTriple(plainText []byte, key []byte, padding int) ([]byte, error) {
	if len(key) != 16 && len(key) != 24 {
		return nil, gberror.NewCode(gbcode.CodeInvalidParameter, "key length error")
	}
	...
	if err != nil {
		err = gberror.WrapCodef(gbcode.CodeInvalidParameter, err, `des.NewTripleDESCipher failed for key "%s"`, newKey)
		return nil, err
	}
...
func EncryptCBC(plainText []byte, key []byte, iv []byte, padding int) ([]byte, error) {
	...
	if len(iv) != block.BlockSize() {
		return nil, gberror.NewCode(gbcode.CodeInvalidParameter, "invalid iv length")
	}
...
func DecryptCBC(cipherText []byte, key []byte, iv []byte, padding int) ([]byte, error) {
	...
	if len(iv) != block.BlockSize() {
		return nil, gberror.NewCode(gbcode.CodeInvalidParameter, "iv length invalid")
	}
...
func EncryptCBCTriple(plainText []byte, key []byte, iv []byte, padding int) ([]byte, error) {
	if len(key) != 16 && len(key) != 24 {
		return nil, gberror.NewCode(gbcode.CodeInvalidParameter, "key length invalid")
	}
...
func Padding(text []byte, padding int) ([]byte, error) {
	...
	case NOPADDING:
		if len(text)%8 != 0 {
			return nil, gberror.NewCode(gbcode.CodeInvalidParameter, "invalid text length")
		}
	...
	default:
		return nil, gberror.NewCodef(gbcode.CodeInvalidParameter, `unsupported padding type "%d"`, padding)
	}
