...
type internalConfig struct {
	rotatedHandlerInitialized *gbtype.Bool // Whether the rotation feature initialized.
}
...
func DefaultConfig() Config {
	c := Config{
		...
		internalConfig: internalConfig{
			rotatedHandlerInitialized: gbtype.NewBool(),
		},
	}
...
func (l *Logger) SetConfigWithMap(m map[string]interface{}) error {
	...
	rotateSizeKey, rotateSizeValue := gbutil.MapPossibleItemByKey(m, "RotateSize")
	if rotateSizeValue != nil {
		m[rotateSizeKey] = gbfile.StrToSize(gbconv.String(rotateSizeValue))
		if m[rotateSizeKey] == -1 {
			return gberror.NewCodef(gbcode.CodeInvalidConfiguration, `invalid rotate size: %v`, rotateSizeValue)
		}
	}
	if err := gbconv.Struct(m, &l.config); err != nil {
		return err
	}
...
func (l *Logger) SetPath(path string) error {
	if path == "" {
		return gberror.NewCode(gbcode.CodeInvalidParameter, "logging path is empty")
	}
	if !gbfile.Exists(path) {
		if err := gbfile.Mkdir(path); err != nil {
			return gberror.Wrapf(err, `Mkdir "%s" failed in PWD "%s"`, path, gbfile.Pwd())
		}
	}
	l.config.Path = strings.TrimRight(path, gbfile.Separator)
	return nil
}
