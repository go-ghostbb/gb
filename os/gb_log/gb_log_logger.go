...
import (
	...
	gbfpool "github.com/Ghostbb-io/gb/os/gb_fpool"
	gbmlock "github.com/Ghostbb-io/gb/os/gb_mlock"
...
func (l *Logger) getFilePath(now time.Time) string {
	// Content containing "{}" in the file name is formatted using gbtime.
	file, _ := gbregex.ReplaceStringFunc(`{.+?}`, l.config.File, func(s string) string {
		return gbtime.New(now).Format(strings.Trim(s, "{}"))
	})
	file = gbfile.Join(l.config.Path, file)
	return file
}
...
func (l *Logger) print(ctx context.Context, level int, stack string, values ...any) {
	...
	if l.config.Flags&(F_FILE_LONG|F_FILE_SHORT|F_CALLER_FN) > 0 {
		callerFnName, path, line := gbdebug.CallerWithFilter(
		...
		if line >= 0 && len(path) > 1 {
			...
			if l.config.Flags&F_FILE_SHORT > 0 {
				input.CallerPath = fmt.Sprintf(`%s:%d:`, gbfile.Basename(path), line)
			}
		}
	}
	// Prefix.
		...
		if len(l.config.CtxKeys) > 0 {
			for _, ctxKey := range l.config.CtxKeys {
				...
				if ctxValue = ctx.Value(ctxKey); ctxValue == nil {
					ctxValue = ctx.Value(gbctx.StrKey(gbconv.String(ctxKey)))
				}
				if ctxValue != nil {
					if input.CtxStr != "" {
						input.CtxStr += ", "
					}
					input.CtxStr += gbconv.String(ctxValue)
				}
			}
		}
	}
...
func (l *Logger) printToFile(ctx context.Context, t time.Time, in *HandlerInput) *bytes.Buffer {
	...
	gbmlock.Lock(memoryLockKey)
	defer gbmlock.Unlock(memoryLockKey)

	// Rotation file size checks.
	if l.config.RotateSize > 0 && gbfile.Size(logFilePath) > l.config.RotateSize {
...
func (l *Logger) createFpInPool(ctx context.Context, path string) *gbfpool.File {
	file, err := gbfpool.Open(
		path,
		defaultFileFlags,
		defaultFilePerm,
		defaultFileExpire,
...
func (l *Logger) GetStack(skip ...int) string {
	...
	return gbdebug.StackWithFilters(filters, stackSkip)
}
