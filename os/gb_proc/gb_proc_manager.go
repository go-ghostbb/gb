...
type Manager struct {
	processes *gbmap.IntAnyMap // Process id to Process object mapping.
}

// NewManager creates and returns a new process manager.
func NewManager() *Manager {
	return &Manager{
		processes: gbmap.NewIntAnyMap(true),
	}
}
...
func (m *Manager) SignalAll(sig os.Signal) error {
	for _, p := range m.Processes() {
		if err := p.Signal(sig); err != nil {
			err = gberror.Wrapf(err, `send signal to process failed for pid "%d" with signal "%s"`, p.Process.Pid, sig)
			return err
		}
	}
	return nil
