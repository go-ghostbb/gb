...
func (p *Process) Send(data []byte) error {
	...
	return gberror.NewCode(gbcode.CodeInvalidParameter, "invalid process")
}
...
func (p *Process) Kill() (err error) {
	err = p.Process.Kill()
	if err != nil {
		err = gberror.Wrapf(err, `kill process failed for pid "%d"`, p.Process.Pid)
		return err
	}
	if p.Manager != nil {
