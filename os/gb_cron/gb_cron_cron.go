...
type Cron struct {
	idGen   *gbtype.Int64    // Used for unique name generation.
	status  *gbtype.Int      // Timed task status(0: Not Start; 1: Running; 2: Stopped; -1: Closed)
	entries *gbmap.StrAnyMap // All timed task entries.
	logger  gblog.ILogger    // Logger, it is nil in default.
}

// New returns a new Cron object with default settings.
func New() *Cron {
	return &Cron{
		idGen:   gbtype.NewInt64(),
		status:  gbtype.NewInt(StatusRunning),
		entries: gbmap.NewStrAnyMap(true),
	}
}

// SetLogger sets the logger for cron.
func (c *Cron) SetLogger(logger gblog.ILogger) {
	c.logger = logger
}

// GetLogger returns the logger in the cron.
func (c *Cron) GetLogger() gblog.ILogger {
	return c.logger
}
...
func (c *Cron) DelayAddEntry(ctx context.Context, delay time.Duration, pattern string, job JobFunc, times int, isSingleton bool, name ...string) {
	gbtimer.AddOnce(ctx, delay, func(ctx context.Context) {
...
func (c *Cron) Entries() []*Entry {
	array := gbarray.NewSortedArraySize(c.entries.Size(), func(v1, v2 interface{}) int {
		entry1 := v1.(*Entry)
		entry2 := v2.(*Entry)
