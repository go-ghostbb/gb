...
type cronSchedule struct {
	...
	lastTimestamp   *gbtype.Int64    // Last timestamp number, for timestamp fix in some delay.
}
...
func newSchedule(pattern string) (*cronSchedule, error) {
	...
	if match, _ := gbregex.MatchString(`(@\w+)\s*(\w*)\s*`, pattern); len(match) > 0 {
		key := strings.ToLower(match[1])
		if v, ok := predefinedPatternMap[key]; ok {
			pattern = v
		} else if strings.Compare(key, "@every") == 0 {
			d, err := gbtime.ParseDuration(match[2])
			if err != nil {
				return nil, err
			}
			return &cronSchedule{
				...
				pattern:         pattern,
				lastTimestamp:   gbtype.NewInt64(currentTimestamp),
			}, nil
		} else {
			return nil, gberror.NewCodef(gbcode.CodeInvalidParameter, `invalid pattern: "%s"`, pattern)
		}
	}
	// Handle the common cron pattern, like:
	// 0 0 0 1 1 2
	if match, _ := gbregex.MatchString(regexForCron, pattern); len(match) == 7 {
	...
	for _, itemElem := range strings.Split(item, ",") {
		...
		if len(intervalArray) == 2 {
			if number, err := strconv.Atoi(intervalArray[1]); err != nil {
				return nil, gberror.NewCodef(gbcode.CodeInvalidParameter, `invalid pattern item: "%s"`, itemElem)
			} else {
				interval = number
			}
		}
...
func parsePatternItemValue(value string, itemType int) (int, error) {
	if gbregex.IsMatchString(`^\d+$`, value) {
		// It is pure number.
	...
	return 0, gberror.NewCodef(gbcode.CodeInvalidParameter, `invalid pattern value: "%s"`, value)
}
...
func (s *cronSchedule) checkMeetAndUpdateLastSeconds(ctx context.Context, t time.Time) bool {
	var (
		...
		lastTime      = gbtime.NewFromTimeStamp(lastTimestamp)
	)
