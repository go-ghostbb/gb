...
func Test_Format(t *testing.T) {
	gbtest.C(t, func(t *gbtest.T) {
		timeTemp, err := gbtime.StrToTime("2006-01-11 15:04:05", "Y-m-d H:i:s")
		...
		//补零
		timeTemp1, err := gbtime.StrToTime("2006-01-02 03:04:05", "Y-m-d H:i:s")
		if err != nil {
			t.Error("test fail")
		}
		t.Assert(timeTemp1.Format("Y-m-d h:i:s"), "2006-01-02 03:04:05")
		//不补零
		timeTemp2, err := gbtime.StrToTime("2006-01-02 03:04:05", "Y-m-d H:i:s")
		if err != nil {
			t.Error("test fail")
		}
		...
		for _, v := range times {
			t1, err1 := gbtime.StrToTime(v["k"], "Y-m-d")
			t.Assert(err1, nil)
...
func Test_Format_ZeroString(t *testing.T) {
	gbtest.C(t, func(t *gbtest.T) {
		timeTemp, err := gbtime.StrToTime("0000-00-00 00:00:00")
		t.AssertNE(err, nil)
