// Package gbtime provides functionality for measuring and displaying time.
//
...
func StrToTime(str string, format ...string) (*Time, error) {
	...
	if match = timeRegex1.FindStringSubmatch(str); len(match) > 0 && match[1] != "" {
		...
		return nil, gberror.NewCodef(gbcode.CodeInvalidParameter, `unsupported time converting for string "%s"`, str)
	}
	...
	if match[6] != "" {
		...
		if len(zone) <= 6 {
			...
			if h > 24 || m > 59 || s > 59 {
				return nil, gberror.NewCodef(gbcode.CodeInvalidParameter, `invalid zone string "%s"`, match[6])
			}
			operation := match[5]
	...
	if month <= 0 || day <= 0 {
		return nil, gberror.NewCodef(gbcode.CodeInvalidParameter, `invalid time string "%s"`, str)
	}
...
func ConvertZone(strTime string, toZone string, fromZone ...string) (*Time, error) {
	...
	if len(fromZone) > 0 {
		if l, err = time.LoadLocation(fromZone[0]); err != nil {
			err = gberror.WrapCodef(gbcode.CodeInvalidParameter, err, `time.LoadLocation failed for name "%s"`, fromZone[0])
			return nil, err
		} else {
	...
	if l, err = time.LoadLocation(toZone); err != nil {
		err = gberror.WrapCodef(gbcode.CodeInvalidParameter, err, `time.LoadLocation failed for name "%s"`, toZone)
		return nil, err
	} else {
...
func StrToTimeLayout(str string, layout string) (*Time, error) {
	if t, err := time.ParseInLocation(layout, str, time.Local); err == nil {
		...
		return nil, gberror.WrapCodef(
			gbcode.CodeInvalidParameter, err,
...
func ParseTimeFromContent(content string, format ...string) *Time {
	...
	if len(format) > 0 {
		for _, item := range format {
			match, err = gbregex.MatchString(formatToRegexPattern(item), content)
			if err != nil {
...
func ParseDuration(s string) (duration time.Duration, err error) {
	...
	if utils.IsNumeric(s) {
		...
		if err != nil {
			err = gberror.WrapCodef(gbcode.CodeInvalidParameter, err, `strconv.ParseInt failed for string "%s"`, s)
			return 0, err
		}
		return time.Duration(num), nil
	}
	match, err := gbregex.MatchString(`^([\-\d]+)[dD](.*)$`, s)
	if err != nil {
		return 0, err
	}
	if len(match) == 3 {
		...
		if err != nil {
			err = gberror.WrapCodef(gbcode.CodeInvalidParameter, err, `strconv.ParseInt failed for string "%s"`, match[1])
			return 0, err
		}
		s = fmt.Sprintf(`%dh%s`, num*24, match[2])
		duration, err = time.ParseDuration(s)
		if err != nil {
			err = gberror.WrapCodef(gbcode.CodeInvalidParameter, err, `time.ParseDuration failed for string "%s"`, s)
		}
		return
	}
	duration, err = time.ParseDuration(s)
