// Package gbcfg provides reading, caching and managing for configuration.
package gbcfg
...
func (c *Config) Get(ctx context.Context, pattern string, def ...interface{}) (*gbvar.Var, error) {
	var (
		err   error
		value interface{}
	)
...
func (c *Config) GetWithEnv(ctx context.Context, pattern string, def ...interface{}) (*gbvar.Var, error) {
	value, err := c.Get(ctx, pattern)
	if err != nil && gberror.Code(err) != gbcode.CodeNotFound {
		return nil, err
	}
	if value == nil {
		if v := gbenv.Get(utils.FormatEnvKey(pattern)); v != nil {
			return v, nil
		}
		if len(def) > 0 {
			return gbvar.New(def[0]), nil
...
func (c *Config) MustGet(ctx context.Context, pattern string, def ...interface{}) *gbvar.Var {
...
func (c *Config) MustGetWithEnv(ctx context.Context, pattern string, def ...interface{}) *gbvar.Var {
...
func (c *Config) MustGetWithCmd(ctx context.Context, pattern string, def ...interface{}) *gbvar.Var {
