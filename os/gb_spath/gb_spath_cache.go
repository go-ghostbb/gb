...
func (sp *SPath) nameFromPath(filePath, rootPath string) string {
	name := gbstr.Replace(filePath, rootPath, "")
...
func (sp *SPath) addToCache(filePath, rootPath string) {
	...
	if idDir {
		if files, err := gbfile.ScanDir(filePath, "*", true); err == nil {
			// fmt.Println("gspath add to cache:", filePath, files)
			for _, path := range files {
				sp.cache.SetIfNotExist(sp.nameFromPath(path, rootPath), sp.makeCacheValue(path, gbfile.IsDir(path)))
			}
		}
	}
}

// addMonitorByPath adds gbfsnotify monitoring recursively.
...
func (sp *SPath) addMonitorByPath(path string) {
	...
	_, _ = gbfsnotify.Add(path, func(event *gbfsnotify.Event) {
		// gblog.Debug(event.String())
		switch {
		case event.IsRemove():
			sp.cache.Remove(sp.nameFromPath(event.Path, path))

		case event.IsRename():
			if !gbfile.Exists(event.Path) {
				sp.cache.Remove(sp.nameFromPath(event.Path, path))
			}
...
// removeMonitorByPath removes gbfsnotify monitoring of `path` recursively.
func (sp *SPath) removeMonitorByPath(path string) {
	if sp.cache == nil {
		return
	}
	_ = gbfsnotify.Remove(path)
}
