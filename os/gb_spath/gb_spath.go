// Package gbspath implements file index and search for folders.
//
...
type SPath struct {
	paths *gbarray.StrArray // The searching directories array.
	cache *gbmap.StrStrMap  // Searching cache map, it is not enabled if it's nil.
}
...
var (
	...
	pathsMap = gbmap.NewStrAnyMap(true)
)

// New creates and returns a new path searching manager.
func New(path string, cache bool) *SPath {
	sp := &SPath{
		paths: gbarray.NewStrArray(true),
	}
	if cache {
		sp.cache = gbmap.NewStrStrMap(true)
	}
	if len(path) > 0 {
...
func (sp *SPath) Set(path string) (realPath string, err error) {
	...
	if realPath == "" {
		...
		if realPath == "" {
			realPath = gbfile.RealPath(gbfile.Pwd() + gbfile.Separator + path)
		}
	}
	if realPath == "" {
		return realPath, gberror.NewCodef(gbcode.CodeInvalidParameter, `path "%s" does not exist`, path)
	}
	// The set path must be a directory.
	if gbfile.IsDir(realPath) {
		realPath = strings.TrimRight(realPath, gbfile.Separator)
		...
		return "", gberror.NewCode(gbcode.CodeInvalidParameter, path+" should be a folder")
	}
}
...
func (sp *SPath) Search(name string, indexFiles ...string) (filePath string, isDir bool) {
	...
	if sp.cache == nil {
		sp.paths.LockFunc(func(array []string) {
			path := ""
			for _, v := range array {
				path = gbfile.Join(v, name)
				if stat, err := os.Stat(path); stat != nil && !os.IsNotExist(err) {
					path = gbfile.Abs(path)
		...
		if len(indexFiles) > 0 && isDir {
			...
			for _, file := range indexFiles {
				path = filePath + gbfile.Separator + file
