...
func (c *Command) doAddCommand(command *Command) error {
	command.Name = gbstr.Trim(command.Name)
	if command.Name == "" {
		return gberror.New("command name should not be empty")
	}
	// Repeated check.
	var (
		commandNameSet = gbset.NewStrSet()
	)
	for _, cmd := range c.commands {
		commandNameSet.Add(cmd.Name)
	}
	if commandNameSet.Contains(command.Name) {
		return gberror.Newf(`command "%s" is already added to command "%s"`, command.Name, c.Name)
	}
