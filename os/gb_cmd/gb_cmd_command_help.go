...
func (c *Command) PrintTo(writer io.Writer) {
	...
	if len(c.commands) > 0 {
		...
		for _, cmd := range c.commands {
			var (
				...
				wordwrapPrefix = gbstr.Repeat(" ", len(prefix+cmd.Name)+spaceLength+4)
			)
			c.printLineBrief(printLineBriefInput{
				Buffer:         buffer,
				Name:           cmd.Name,
				Prefix:         prefix,
				Brief:          gbstr.Trim(cmd.Brief),
	...
	if c.hasArgumentFromIndex() {
		...
		for _, arg := range arguments {
			...
			var (
				...
				wordwrapPrefix = gbstr.Repeat(" ", len(prefix+arg.Name)+spaceLength+4)
			)
			c.printLineBrief(printLineBriefInput{
				Buffer:         buffer,
				Name:           arg.Name,
				Prefix:         prefix,
				Brief:          gbstr.Trim(arg.Brief),
				WordwrapPrefix: wordwrapPrefix,
				SpaceLength:    spaceLength,
			})
		}
	...
	if c.hasArgumentFromOption() {
		...
		for _, arg := range arguments {
			...
			var (
				...
				wordwrapPrefix = gbstr.Repeat(" ", len(prefix+nameStr)+spaceLength+4)
			)
			c.printLineBrief(printLineBriefInput{
				Buffer:         buffer,
				Name:           nameStr,
	...
	if c.Examples != "" {
		...
		for _, line := range gbstr.SplitAndTrim(gbstr.Trim(c.Examples), "\n") {
			buffer.WriteString(prefix)
			buffer.WriteString(gbstr.WordWrap(gbstr.Trim(line), maxLineChars, "\n"+prefix))
			buffer.WriteString("\n")
		}
		buffer.WriteString("\n")
	}
	...
	if c.Description != "" {
		...
		for _, line := range gbstr.SplitAndTrim(gbstr.Trim(c.Description), "\n") {
			buffer.WriteString(prefix)
			buffer.WriteString(gbstr.WordWrap(gbstr.Trim(line), maxLineChars, "\n"+prefix))
			buffer.WriteString("\n")
		}
	...
	// Additional.
	if c.Additional != "" {
		lineStr := gbstr.WordWrap(gbstr.Trim(c.Additional), maxLineChars, "\n")
		buffer.WriteString(lineStr)
		buffer.WriteString("\n")
	}
	content := buffer.String()
	content = gbstr.Replace(content, "\t", "    ")
...
func (c *Command) printLineBrief(in printLineBriefInput) {
	briefArray := gbstr.SplitAndTrim(in.Brief, "\n")
	if len(briefArray) == 0 {
	...
	for i, line := range briefArray {
		...
		if i == 0 {
			lineStr = fmt.Sprintf(
				"%s%s%s%s\n",
				in.Prefix, in.Name, gbstr.Repeat(" ", in.SpaceLength+4), line,
			)
		} else {
			lineStr = fmt.Sprintf(
				"%s%s%s%s\n",
				in.Prefix, gbstr.Repeat(" ", len(in.Name)), gbstr.Repeat(" ", in.SpaceLength+4), line,
			)
		}
		lineStr = gbstr.WordWrap(lineStr, maxLineChars, "\n"+in.WordwrapPrefix)
