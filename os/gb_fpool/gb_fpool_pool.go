...
func newFilePool(p *Pool, path string, flag int, perm os.FileMode, ttl time.Duration) *gbpool.Pool {
	pool := gbpool.New(ttl, func() (interface{}, error) {
		file, err := os.OpenFile(path, flag, perm)
		if err != nil {
			err = gberror.Wrapf(err, `os.OpenFile failed for file "%s", flag "%d", perm "%s"`, path, flag, perm)
			return nil, err
		}
		return &File{
			File: file,
...
func (p *Pool) File() (*File, error) {
	if v, err := p.pool.Get(); err != nil {
		...
		if !p.init.Val() && p.init.Cas(false, true) {
			_, _ = gbfsnotify.Add(f.path, func(event *gbfsnotify.Event) {
