...
type AdapterMemory struct {
	...
	lruGetList  *gblist.List              // lruGetList is the LRU history according to Get function.
	eventList   *gblist.List              // eventList is the asynchronous event list for internal data synchronization.
	closed      *gbtype.Bool              // closed controls the cache closed or not.
}
...
func NewAdapterMemory(lruCap ...int) Adapter {
	...
	gbtimer.AddSingleton(context.Background(), time.Second, c.syncEventAndClearExpired)
	return c
}
...
func (c *AdapterMemory) Get(ctx context.Context, key interface{}) (*gbvar.Var, error) {
...
func (c *AdapterMemory) GetOrSet(ctx context.Context, key interface{}, value interface{}, duration time.Duration) (*gbvar.Var, error) {
	v, err := c.Get(ctx, key)
	if err != nil {
		return nil, err
	}
...
// It deletes the `key` if `duration` < 0 or given `value` is nil, but it does nothing
// if `value` is a function and the function result is nil.
func (c *AdapterMemory) GetOrSetFunc(ctx context.Context, key interface{}, f Func, duration time.Duration) (*gbvar.Var, error) {
	v, err := c.Get(ctx, key)
	if err != nil {
		return nil, err
	}
...
// writing mutex lock for concurrent safety purpose.
func (c *AdapterMemory) GetOrSetFuncLock(ctx context.Context, key interface{}, f Func, duration time.Duration) (*gbvar.Var, error) {
	v, err := c.Get(ctx, key)
	if err != nil {
		return nil, err
	}
...
func (c *AdapterMemory) GetExpire(ctx context.Context, key interface{}) (time.Duration, error) {
	if item, ok := c.data.Get(key); ok {
		return time.Duration(item.e-gbtime.TimestampMilli()) * time.Millisecond, nil
	}
	return -1, nil
}
...
func (c *AdapterMemory) Remove(ctx context.Context, keys ...interface{}) (*gbvar.Var, error) {
	...
	for _, key := range removedKeys {
		c.eventList.PushBack(&adapterMemoryEvent{
			k: key,
			e: gbtime.TimestampMilli() - 1000000,
		})
	}
	return gbvar.New(value), nil
}
...
func (c *AdapterMemory) Update(ctx context.Context, key interface{}, value interface{}) (oldValue *gbvar.Var, exist bool, err error) {
	v, exist, err := c.data.Update(key, value)
	return gbvar.New(v), exist, err
}
...
func (c *AdapterMemory) doSetWithLockCheck(ctx context.Context, key interface{}, value interface{}, duration time.Duration) (result *gbvar.Var, err error) {
...
func (c *AdapterMemory) getInternalExpire(duration time.Duration) int64 {
	...
	return gbtime.TimestampMilli() + duration.Nanoseconds()/1000000
}
...
func (c *AdapterMemory) syncEventAndClearExpired(ctx context.Context) {
	...
	var (
		expireSet *gbset.Set
		ek        = c.makeExpireKey(gbtime.TimestampMilli())
