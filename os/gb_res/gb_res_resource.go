...
import (
	"context"
	"fmt"
	gbtree "github.com/Ghostbb-io/gb/container/gb_tree"
...
func New() *Resource {
	return &Resource{
		tree: gbtree.NewBTree(defaultTreeM, func(v1, v2 interface{}) int {
...
func (r *Resource) Load(path string, prefix ...string) error {
	...
	return r.Add(gbfile.GetContents(realPath), prefix...)
}
...
func (r *Resource) doScanDir(path string, pattern string, recursive bool, onlyFile bool) []*File {
	...
	r.tree.IteratorFrom(path, true, func(key, value interface{}) bool {
		...
		for _, p := range patterns {
			if match, err := filepath.Match(p, gbfile.Basename(name)); err == nil && match {
...
func (r *Resource) Export(src, dst string, option ...ExportOption) error {
	...
	for _, file := range files {
		...
		if exportOption.RemovePrefix != "" {
			name = gbstr.TrimLeftStr(name, exportOption.RemovePrefix)
		}
		name = gbstr.Trim(name, `\/`)
		if name == "" {
			continue
		}
		path = gbfile.Join(dst, name)
		if file.FileInfo().IsDir() {
			err = gbfile.Mkdir(path)
		} else {
			err = gbfile.PutBytes(path, file.Content())
		}
		if err != nil {
			return err
		}
...
func (r *Resource) Dump() {
	...
	r.tree.Iterator(func(key, value interface{}) bool {
		...
		fmt.Printf(
			"%v %8s %s\n",
			gbtime.New(info.ModTime()).ISO8601(),
			gbfile.FormatSize(info.Size()),
			key,
		)
		return true
	})
