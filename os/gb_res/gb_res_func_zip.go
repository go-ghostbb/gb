...
import (
	...
	"ghostbb.io/gb/internal/fileinfo"
...
func doZipPathWriter(srcPath string, zipWriter *zip.Writer, option ...Option) error {
	...
	absolutePath, err = gbfile.Search(srcPath)
	if err != nil {
		return err
	}
	if gbfile.IsDir(absolutePath) {
		files, err = gbfile.ScanDir(absolutePath, "*", true)
		if err != nil {
			return err
		}
	} else {
	...
	if headerPrefix != "" && gbfile.IsDir(absolutePath) {
		headerPrefix += "/"
	}
	...
	if headerPrefix == "" {
		if usedOption.KeepPath {
			...
			headerPrefix = gbfile.Basename(absolutePath)
		}
	}
	...
	for _, file := range files {
		...
		if subFilePath != "" {
			subFilePath = gbfile.Dir(subFilePath)
		}
	...
	if headerPrefix != "" {
		...
		for {
			name = strings.ReplaceAll(gbfile.Basename(tmpPath), `\`, `/`)
...
func zipFile(path string, prefix string, zw *zip.Writer) error {
	...
	if err != nil {
		err = gberror.Wrapf(err, `os.Open failed for path "%s"`, path)
		return nil
	}
	defer file.Close()
	...
	info, err := file.Stat()
	if err != nil {
		err = gberror.Wrapf(err, `read file stat failed for path "%s"`, path)
		return err
	}
	...
	if err != nil {
		err = gberror.Wrapf(err, `create zip header failed for %#v`, header)
		return err
	}
	if !info.IsDir() {
		if _, err = io.Copy(writer, file); err != nil {
			err = gberror.Wrapf(err, `io.Copy failed for file "%s"`, path)
			return err
		}
	}
	return nil
...
func createFileHeader(info os.FileInfo, prefix string) (*zip.FileHeader, error) {
	...
	if err != nil {
		err = gberror.Wrapf(err, `create file header failed for name "%s"`, info.Name())
		return nil, err
	}
	if len(prefix) > 0 {
		header.Name = prefix + `/` + header.Name
		header.Name = strings.ReplaceAll(header.Name, `\`, `/`)
		header.Name, _ = gbregex.ReplaceString(`/{2,}`, `/`, header.Name)
	}
	return header, nil
}
