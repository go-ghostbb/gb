...
func (t *Timer) createEntry(in createEntryInput) *Entry {
	...
	var (
		entry = &Entry{
			...
			status:      gbtype.NewInt(in.Status),
			isSingleton: gbtype.NewBool(in.IsSingleton),
			nextTicks:   gbtype.NewInt64(nextTicks),
			infinite:    gbtype.NewBool(infinite),
		}
	)
