...
type Entry struct {
	...
	times       *gbtype.Int     // Limit running times.
	status      *gbtype.Int     // Job status.
	isSingleton *gbtype.Bool    // Singleton mode.
	nextTicks   *gbtype.Int64   // Next run ticks of the job.
			...
			if exception := recover(); exception != nil {
				if exception != panicExit {
					if v, ok := exception.(error); ok && gberror.HasStack(v) {
						...
						panic(gberror.NewCodef(gbcode.CodeInternalPanic, "exception recovered: %+v", exception))
					}
				} else {
					entry.Close()
					return
