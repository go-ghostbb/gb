...
import (
	...
	gbsel "ghostbb.io/gb/net/gb_sel"
...
func (c *Client) SetHeaderRaw(headers string) *Client {
	for _, line := range gbstr.SplitAndTrim(headers, "\n") {
		array, _ := gbregex.MatchString(httpRegexHeaderRaw, line)
		if len(array) >= 3 {
...
func (c *Client) SetTLSKeyCrt(crtFile, keyFile string) error {
	...
	if err != nil {
		return gberror.Wrap(err, "LoadKeyCrt failed")
	}
	if v, ok := c.Transport.(*http.Transport); ok {
	...
	return gberror.New(`cannot set TLSClientConfig for custom Transport of the client`)
}
...
func (c *Client) SetBuilder(builder gbsel.Builder) {
	c.builder = builder
}

// SetDiscovery sets the load balance builder for client.
func (c *Client) SetDiscovery(discovery gbsvc.Discovery) {
