...
func (b *Baggage) SetValue(key string, value interface{}) context.Context {
	member, _ := baggage.NewMember(key, gbconv.String(value))
...
func (b *Baggage) SetMap(data map[string]interface{}) context.Context {
	...
	for k, v := range data {
		member, _ := baggage.NewMember(k, gbconv.String(v))
...
func (b *Baggage) GetMap() *gbmap.StrAnyMap {
	m := gbmap.NewStrAnyMap()
...
func (b *Baggage) GetVar(key string) *gbvar.Var {
