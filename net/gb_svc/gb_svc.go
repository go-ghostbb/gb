// Package gbsvc provides service registry and discovery definition.
package gbsvc
...
const (
	...
	EnvPrefix                 = `GB_SVC_PREFIX`     // EnvPrefix is the environment variable prefix.
	EnvDeployment             = `GB_SVC_DEPLOYMENT` // EnvDeployment is the environment variable deployment.
	EnvNamespace              = `GB_SVC_NAMESPACE`  // EnvNamespace is the environment variable namespace.
	EnvName                   = `GB_SVC_Name`       // EnvName is the environment variable name.
	EnvVersion                = `GB_SVC_VERSION`    // EnvVersion is the environment variable version.
...
func SetRegistry(registry Registry) {
	if registry == nil {
		panic(gberror.New(`invalid Registry value "nil" given`))
	}
