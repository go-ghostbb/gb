...
func Marshal(v interface{}) (marshaledBytes []byte, err error) {
	...
	if err != nil {
		err = gberror.Wrap(err, `json.Marshal failed`)
	}
	return
}
...
func MarshalIndent(v interface{}, prefix, indent string) (marshaledBytes []byte, err error) {
	...
	if err != nil {
		err = gberror.Wrap(err, `json.MarshalIndent failed`)
	}
	return
}
...
func Unmarshal(data []byte, v interface{}) (err error) {
	...
	if err != nil {
		err = gberror.Wrap(err, `json.Unmarshal failed`)
	}
	return
}
...
func UnmarshalUseNumber(data []byte, v interface{}) (err error) {
	...
	if err != nil {
		err = gberror.Wrap(err, `json.UnmarshalUseNumber failed`)
	}
	return
}
