...
type AdapterOperation interface {
	...
	Do(ctx context.Context, command string, args ...interface{}) (*gbvar.Var, error)
...
type Conn interface {
	...
	// Do send a command to the server and returns the received reply.
	// It uses json.Marshal for struct/slice/map type values before committing them to redis.
	Do(ctx context.Context, command string, args ...interface{}) (result *gbvar.Var, err error)
...
type ConnCommand interface {
	...
	Receive(ctx context.Context) (result *gbvar.Var, err error)
}
