...
var (
	...
	localConfigMap = gbmap.NewStrAnyMap(true)
)
...
func ConfigFromMap(m map[string]interface{}) (config *Config, err error) {
	config = &Config{}
	if err = gbconv.Scan(m, config); err != nil {
		err = gberror.NewCodef(gbcode.CodeInvalidConfiguration, `invalid redis configuration: %#v`, m)
	}
