// Package gbpool provides object-reusable concurrent-safe pool.
package gbpool
...
type Pool struct {
	list    *gblist.List                // Available/idle items list.
	closed  *gbtype.Bool                // Whether the pool is closed.
...
func New(ttl time.Duration, newFunc NewFunc, expireFunc ...ExpireFunc) *Pool {
	...
	gbtimer.AddSingleton(context.Background(), time.Second, r.checkExpireItems)
	return r
}
...
func (p *Pool) Put(value interface{}) error {
	if p.closed.Val() {
		return gberror.NewCode(gbcode.CodeInvalidOperation, "pool is closed")
	}
	item := &poolItem{
		value: value,
	}
	if p.TTL == 0 {
		...
		item.expireAt = gbtime.TimestampMilli() + p.TTL.Nanoseconds()/1000000
	}
...
func (p *Pool) Get() (interface{}, error) {
	for !p.closed.Val() {
		if r := p.list.PopFront(); r != nil {
			f := r.(*poolItem)
			if f.expireAt == 0 || f.expireAt > gbtime.TimestampMilli() {
				return f.value, nil
	...
	return nil, gberror.NewCode(gbcode.CodeInvalidOperation, "pool is empty")
}
...
func (p *Pool) checkExpireItems(ctx context.Context) {
	...
	var timestampMilli = gbtime.TimestampMilli()
	for {
