// Package gbset provides kinds of concurrent-safe/unsafe sets.
package gbset
...
func NewFrom(items interface{}, safe ...bool) *Set {
	...
	for _, v := range gbconv.Interfaces(items) {
		m[v] = struct{}{}
	}
	return &Set{
		data: m,
...
func (set *Set) Join(glue string) string {
	...
	for k := range set.data {
		buffer.WriteString(gbconv.String(k))
		if i != l-1 {
...
func (set *Set) UnmarshalValue(value interface{}) (err error) {
	...
	switch value.(type) {
	case string, []byte:
		err = json.UnmarshalUseNumber(gbconv.Bytes(value), &array)
	default:
		array = gbconv.SliceAny(value)
	}
	for _, v := range array {
		set.data[v] = struct{}{}
	}
