...
func (a *Array) Set(index int, value interface{}) error {
	...
	defer a.mu.Unlock()
	if index < 0 || index >= len(a.array) {
		return gberror.NewCodef(gbcode.CodeInvalidParameter, "index %d out of array range %d", index, len(a.array))
	}
	a.array[index] = value
	return nil
}
...
func (a *Array) PopRand() (value interface{}, found bool) {
	a.mu.Lock()
	defer a.mu.Unlock()
	return a.doRemoveWithoutLock(gbrand.Intn(len(a.array)))
}
...
func (a *Array) PopRands(size int) []interface{} {
	...
	for i := 0; i < size; i++ {
		array[i], _ = a.doRemoveWithoutLock(gbrand.Intn(len(a.array)))
	}
	return array
}
...
func (a *Array) Merge(array interface{}) *Array {
	return a.Append(gbconv.Interfaces(array)...)
}
...
func (a *Array) Fill(startIndex int, num int, value interface{}) error {
	...
	if startIndex < 0 || startIndex > len(a.array) {
		return gberror.NewCodef(gbcode.CodeInvalidParameter, "index %d out of array range %d", startIndex, len(a.array))
	}
...
func (a *Array) Rand() (value interface{}, found bool) {
	...
	return a.array[gbrand.Intn(len(a.array))], true
}
...
func (a *Array) Rands(size int) []interface{} {
	...
	array := make([]interface{}, size)
	for i := 0; i < size; i++ {
		array[i] = a.array[gbrand.Intn(len(a.array))]
	}
	return array
}
...
func (a *Array) Shuffle() *Array {
	a.mu.Lock()
	defer a.mu.Unlock()
	for i, v := range gbrand.Perm(len(a.array)) {
...
func (a *Array) Join(glue string) string {
	...
	for k, v := range a.array {
		buffer.WriteString(gbconv.String(v))
		if k != len(a.array)-1 {
...
func (a *Array) String() string {
	...
	for k, v := range a.array {
		s = gbconv.String(v)
		if gbstr.IsNumeric(s) {
			buffer.WriteString(s)
		} else {
			buffer.WriteString(`"` + gbstr.QuoteMeta(s, `"\`) + `"`)
		}
		if k != len(a.array)-1 {
			buffer.WriteByte(',')
		}
...
func (a *Array) UnmarshalValue(value interface{}) error {
	...
	switch value.(type) {
	case string, []byte:
		return json.UnmarshalUseNumber(gbconv.Bytes(value), &a.array)
	default:
		a.array = gbconv.SliceAny(value)
	}
	return nil
}
