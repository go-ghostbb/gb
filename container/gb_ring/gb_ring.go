// Package gbring provides a concurrent-safe/unsafe ring(circular lists).
package gbring
...
type Ring struct {
	...
	len   *gbtype.Int  // Length(already used size).
	cap   *gbtype.Int  // Capability(>=len).
	dirty *gbtype.Bool // Dirty, which means the len and cap should be recalculated. It's marked dirty when the size of ring changes.
}
...
func New(cap int, safe ...bool) *Ring {
	return &Ring{
		...
		ring:  ring.New(cap),
		len:   gbtype.NewInt(),
		cap:   gbtype.NewInt(cap),
		dirty: gbtype.NewBool(),
	}
}
