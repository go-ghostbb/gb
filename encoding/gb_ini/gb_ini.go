// Package gbini provides accessing and converting for INI content.
package gbini
...
func Decode(data []byte) (res map[string]interface{}, err error) {
	...
	for {
		...
		if err != nil {
			...
			err = gberror.Wrapf(err, `bufioReader.ReadString failed`)
			return nil, err
		}
	...
	if !haveSection {
		return nil, gberror.NewCode(gbcode.CodeInvalidParameter, "failed to parse INI file, section not found")
	}
	return res, nil
}
...
func Encode(data map[string]interface{}) (res []byte, err error) {
	...
	for section, item := range data {
		...
		if m, ok = item.(map[string]interface{}); ok {
			...
			if err != nil || n == 0 {
				return nil, gberror.Wrapf(err, "w.WriteString failed")
			}
			for k, v := range m {
	...
	if n, err = w.Read(res); err != nil || n == 0 {
		return nil, gberror.Wrapf(err, "w.Read failed")
	}
	return res, nil
}
