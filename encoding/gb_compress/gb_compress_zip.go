...
func ZipPath(fileOrFolderPaths, dstFilePath string, prefix ...string) error {
	...
	if err != nil {
		err = gberror.Wrapf(err, `os.Create failed for name "%s"`, dstFilePath)
		return err
	}
	defer writer.Close()
	...
	for _, path := range strings.Split(fileOrFolderPaths, ",") {
		...
		if err = doZipPathWriter(path, gbfile.RealPath(dstFilePath), zipWriter, prefix...); err != nil {
			return err
		}
	}
	return nil
...
func doZipPathWriter(fileOrFolderPath string, exclude string, zipWriter *zip.Writer, prefix ...string) error {
	...
	fileOrFolderPath, err = gbfile.Search(fileOrFolderPath)
	if err != nil {
		return err
	}
	if gbfile.IsDir(fileOrFolderPath) {
		files, err = gbfile.ScanDir(fileOrFolderPath, "*", true)
		if err != nil {
			return err
		}
	} else {
	...
	if gbfile.IsDir(fileOrFolderPath) {
		if len(headerPrefix) > 0 {
			...
			headerPrefix = gbfile.Basename(fileOrFolderPath)
		}
	}
	...
	for _, file := range files {
		...
		dir := gbfile.Dir(file[len(fileOrFolderPath):])
		if dir == "." {
			dir = ""
		}
...
func UnZipFile(zippedFilePath, dstFolderPath string, zippedPrefix ...string) error {
	...
	if err != nil {
		err = gberror.Wrapf(err, `zip.OpenReader failed for name "%s"`, dstFolderPath)
		return err
	}
...
func UnZipContent(zippedContent []byte, dstFolderPath string, zippedPrefix ...string) error {
	...
	if err != nil {
		err = gberror.Wrapf(err, `zip.NewReader failed`)
		return err
	}
...
func unZipFileWithReader(reader *zip.Reader, dstFolderPath string, zippedPrefix ...string) error {
	...
	if len(zippedPrefix) > 0 {
		prefix = gbstr.Replace(zippedPrefix[0], `\`, `/`)
	}
	...
	for _, file := range reader.File {
		name = gbstr.Replace(file.Name, `\`, `/`)
		name = gbstr.Trim(name, "/")
		if prefix != "" {
		...
		if len(dstDir) > 0 {
			if _, err := os.Stat(dstDir); os.IsNotExist(err) {
				if err = os.MkdirAll(dstDir, 0755); err != nil {
					err = gberror.Wrapf(err, `os.MkdirAll failed for path "%s"`, dstDir)
					return err
				}
			}
		}
		...
		if err != nil {
			err = gberror.Wrapf(err, `file.Open failed`)
			return err
		}
...
func doCopyForUnZipFileWithReader(file *zip.File, fileReader io.ReadCloser, dstPath string) error {
	...
	if err != nil {
		err = gberror.Wrapf(err, `os.OpenFile failed for name "%s"`, dstPath)
		return err
	}
	defer targetFile.Close()

	if _, err = io.Copy(targetFile, fileReader); err != nil {
		err = gberror.Wrapf(err, `io.Copy failed from "%s" to "%s"`, file.Name, dstPath)
		return err
	}
	return nil
}
...
func zipFile(filePath string, prefix string, zw *zip.Writer) error {
	...
	if err != nil {
		err = gberror.Wrapf(err, `os.Open failed for name "%s"`, filePath)
		return err
	}
	defer file.Close()
	...
	info, err := file.Stat()
	if err != nil {
		err = gberror.Wrapf(err, `file.Stat failed for name "%s"`, filePath)
		return err
	}
	...
	if err != nil {
		err = gberror.Wrapf(err, `zip.Writer.CreateHeader failed for header "%#v"`, header)
		return err
	}
	if !info.IsDir() {
		if _, err = io.Copy(writer, file); err != nil {
			err = gberror.Wrapf(err, `io.Copy failed from "%s" to "%s"`, filePath, header.Name)
			return err
		}
	}
	return nil
...
func createFileHeader(info os.FileInfo, prefix string) (*zip.FileHeader, error) {
	...
	if err != nil {
		err = gberror.Wrapf(err, `zip.FileInfoHeader failed for info "%#v"`, info)
		return nil, err
	}
